...
if( sysdesc =~ "^(Aruba|HP) J[^ ]+ .*Switch" ) {
  os_register_and_report( os:"Aruba/HPE Switch Firmware", cpe:"cpe:/o:arubanetworks:switch_firmware", banner_type:BANNER_TYPE, port:port, proto:"udp", banner:sysdesc, desc:SCRIPT_DESC, runs_key:"unixoide" );
  exit( 0 );
}
...
# nb: More detailed OS Detection covered in gb_epson_printer_snmp_detect.nasl
if( sysdesc =~ "^EPSON " ) {

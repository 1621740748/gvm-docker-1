...
if(description)
{
  ...
  script_tag(name:"insight", value:"The flaw exists due to an improper notification mechanism
  on buffer reallocation and corruption in a file's internal readahead buffer which while
  processing large amounts of data with multiple threads could create a condition where a
  buffer that gets allocated with one thread is reallocated due to a large size of input.");

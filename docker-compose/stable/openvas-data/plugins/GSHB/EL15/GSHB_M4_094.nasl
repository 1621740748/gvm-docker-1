...
if(brokenwww){
  ...
  desc = string('Nikto konnte keinen in der -Open Source Vulnerability Database- aufgeführten oder durch eine CVE Nummer addressierten Fehler finden.');
}else if(nikto != "none" && !brokenwww){
  result = string("nicht erfüllt");
  desc = string('Nikto hat folgende in der -Open Source Vulnerability Database- aufgeführten oder durch eine CVE Nummer addressierten Fehler gefunden:\n' + nikto);
}

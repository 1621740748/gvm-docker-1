...
if(description)
{
  ...
  script_name("ManageEngine OpManager <= 11.6 Multiple Vulnerabilities");

  script_tag(name:"summary", value:"ManageEngine OpManager is prone to multiple vulnerabilities.");

  script_tag(name:"vuldetect", value:"Sends a crafted HTTP POST request and checks if it is possible
  to login with default credentials.");

  script_tag(name:"insight", value:"Multiple flaws are due to it was possible to login with default
  credentials: IntegrationUser/plugin.");
...
if(!get_app_location(port:port, cpe:port))
  exit(0);
